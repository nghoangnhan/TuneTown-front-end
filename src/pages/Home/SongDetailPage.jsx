...
const SongDetailPage = () => {
    ...
    useEffect(() => {
        if (!songDetail || !songDetail.poster) return;
                            ...
                            {songDetail.artists?.map((artist) => {
                                return (
                                    <div className="flex flex-row items-center gap-2" key={artist.id}>
                                        <img src={artist.avatar ? artist.avatar : Base_AVA} alt="artist-avatar" className="w-10 h-10 rounded-md" />
